...
public function submitTable(Request $request)
	{
		...
		$newpaycount = $request->input('newpaycount');
		$oldPayRow = $request->input('oldPayRow');
		$paymentMods = $request->input(key: 'payment_mode');
		$paymentAmts = $request->input(key: 'amount');
		$paymentDetails = $request->input(key: 'details');
		...
		/* Part_payment_details*/
		/* for new payment row
		1. add in part_payment details*/
		$totalpayrow = $oldPayRow + $newpaycount;
		$last_payment_details = "";
		$lastpaymentid = "";
		//$lastpaymentDetails = "";
		if ($newpaycount > 0) {
			//dd($oldPayRow);
			for ($i = $oldPayRow; $i <= $totalpayrow - 1; $i++) {
				$partialPaymentDetail = new part_payment_details();
				//dd($i);
				//dd($paymentMods[$i]);
				$partialPaymentDetail->payment_type ="2"; /* assuming it's part payment*/
				$partialPaymentDetail->payment_mode = $paymentMods[$i];
				$partialPaymentDetail->payment_details = $paymentDetails[$i];
				$last_payment_details = $paymentDetails[$i];
				$partialPaymentDetail->partpayment_amount = $paymentAmts[$i];
				...
				Bill_details::where('bill_no', '=',  $request->billno)->update([
